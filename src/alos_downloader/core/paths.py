...
LEGACY_PRODUCT_DIR = "JableTV Downloader"
WATCH_STATE_DIR = ".alos-watch"
LEGACY_WATCH_STATE_DIR = ".Jable_smalltool"
...
def product_data_dir(
    ...
    if migrate:
        migrate_directory(root / LEGACY_PRODUCT_DIR, current)
    return current
...
def select_portable_state_dir(app_dir: Path | str) -> Path:
    ...
    legacy = root / LEGACY_WATCH_STATE_DIR
    if current.is_dir():
        return current
    if legacy.is_dir():
