...
def _session():
    ...
    s = requests.Session()
    try:
        from alos_downloader.core import ssl as ssl_util
...
def check_latest(timeout=10):
    headers = {
        ...
        "User-Agent": "ALOS-Unified-AV-Downloader-Updater",
...
def apply_update_and_restart(new_exe_path):
    ...
    try:
        cur = sys.executable
        pid = os.getpid()
        bat = os.path.join(tempfile.gettempdir(), f'alos_update_{pid}.bat')
        body = f"""@echo off
setlocal
set /a n=0
:retry
