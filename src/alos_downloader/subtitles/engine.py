#!/usr/bin/env python
# coding: utf-8
"""Local-first subtitle generation shared by ALOS Browse and ALOS Watch.
...
from alos_downloader.core.paths import product_data_dir
from alos_downloader.metadata import PRODUCT_NAME
from alos_downloader.sites.base import locate_ffmpeg
from alos_downloader.core.ssl import SharedSSLAdapter
from alos_downloader.core.video_identity import normalize_source_subtitle_evidence
...
REAZONSPEECH_PACK_NAME = (
    f'ALOS_reazonspeech_asr_v{REAZONSPEECH_PACK_VERSION}.zip')
REAZONSPEECH_PACK_RELEASE_TAG = 'v3.0.0'
REAZONSPEECH_PACK_URL = (
    'https://github.com/Alos21750/ALOS-Unified-AV-Downloader/'
...
def _translation_memory_version() -> str:
    from alos_downloader.subtitles.domain import DOMAIN_VERSION
    return (
...
def _run_local_model(
    ...
    destroyed on Windows.  ALOS Watch processes many videos sequentially, so
...
def _to_taiwan_chinese(text: str) -> str:
    ...
    from alos_downloader.subtitles.domain import postprocess_taiwan
...
def _translate_direct(
    ...
    from alos_downloader.subtitles.domain import (
        exact_translation,
        normalize_cue,
        postprocess_english,
    )
...
def _selected_translation_profile():
    ...
    try:
        from alos_downloader.subtitles.settings import get_translation_settings
...
def _translate_api_direct(
    ...
    from alos_downloader.subtitles.llm import (
...
def translate_cues(
    ...
    if (source_language, target_language) == ('ja', 'zh-TW'):
        from alos_downloader.subtitles.domain import exact_translation, normalize_cue
        model_inputs = [_clean_model_text(text) for text in texts]
        normalized = [normalize_cue(text) for text in model_inputs]
