...
<p align="center">
  ...
  Use <strong>ALOS Browse</strong> to browse and pick videos, or <strong>ALOS Watch</strong> to keep watching selected feeds.<br />
...
<p align="center">
  <strong><a href="https://github.com/Alos21750/ALOS-Unified-AV-Downloader/releases/latest/download/ALOS_Browse.exe">Download ALOS Browse</a></strong>
  ·
  <strong><a href="https://github.com/Alos21750/ALOS-Unified-AV-Downloader/releases/latest/download/ALOS_Watch.exe">Download ALOS Watch</a></strong>
  ·
  <a href="https://github.com/Alos21750/ALOS-Unified-AV-Downloader/releases/latest">ALOS Watch portable ZIP (v3.0.0+)</a>
  ·
  <a href="https://github.com/Alos21750/ALOS-Unified-AV-Downloader/releases/latest">View the latest release</a>
</p>

> [!TIP]
> **Fully local by default, with no API key and no uploads.** After a download, ALOS Browse and ALOS Watch can create selectable `.ja.srt`, `.en.srt`, and `.zh-TW.srt` sidecars without modifying the MP4. You can optionally connect a common LLM API; recognized subtitle text is then the only media-derived content sent, alongside required API authentication and ordinary connection metadata—never video or audio.
...
|---|---|---|
| Browse, search, and pick individual videos | **ALOS_Browse.exe** | Browse cards, multi-select, queue, or download now |
| Automatically follow new releases unattended | **ALOS_Watch.exe** | Choose sites, categories, a date, version priority, and schedule; discovery, deduplication, downloading, and subtitles then run automatically |
| Defender reports a detection on the one-file ALOS Watch | **ALOS_Watch_portable.zip** | Verify the hash and detection details before evaluating the fallback that does not temporarily self-extract; stop and report it if the fallback is also detected |
| Run headlessly on a NAS or server | **Docker / CLI** | Pass one or more URLs, or mount a `urls.txt` file |

If you are unsure, start with **ALOS Browse**. Both Windows executables are portable, need no Python installation, and include ffmpeg in the release build.

Learn more: [ALOS Browse](./docs/alos-browse.md) · [ALOS Watch unattended automation](./docs/alos-watch.md) · [AI subtitles](./docs/ai-subtitles.md) · [Docker / CLI](./docs/docker-cli.md) · [v3 migration](./docs/migration-v3.md)

## Windows: start in 30 seconds

1. Download [ALOS_Browse.exe](https://github.com/Alos21750/ALOS-Unified-AV-Downloader/releases/latest/download/ALOS_Browse.exe) or [ALOS_Watch.exe](https://github.com/Alos21750/ALOS-Unified-AV-Downloader/releases/latest/download/ALOS_Watch.exe).
...
## ALOS Browse: browse, pick, download
...
## ALOS Watch: monitor categories automatically
...
1. Choose a destination. If left unset, ALOS Watch creates `tmp` beside the executable.
...
ALOS Watch can check every 1–168 hours or once a day at a specified time using this computer's local time; existing settings continue to default to every 24 hours. Check Now runs one immediate check without creating another recurring schedule. When the same recognized title code appears across categories or sites, the candidate matching your selected version priority is kept. If a code cannot be identified reliably, only an identical URL is deduplicated—ALOS Watch does not guess.

State is stored in `.alos-watch` beside the executable when writable, otherwise in `%APPDATA%\ALOS Unified AV Downloader\watch`.
...
- ALOS Browse keeps video downloads and subtitle processing in separate queues, so background subtitle work does not occupy any of the 1–32 video download slots.
...
| Site | ALOS Browse browse/search/download | ALOS Watch monitoring | Docker/CLI URL download |
|---|:---:|:---:|:---:|
| JableTV | ✓ | ✓ | ✓ |
| MissAV | ✓ | ✓ | ✓ |
| SupJav | ✓ | ✓ | ✓ |
...
The public image is `ghcr.io/alos21750/alos-unified-av-downloader:latest`. GitHub Actions builds both amd64 and arm64 variants.
...
Docker is a headless, run-to-completion download job. It does not contain the ALOS Browse or ALOS Watch GUI.

## Troubleshooting

When opening a [GitHub Issue](https://github.com/Alos21750/ALOS-Unified-AV-Downloader/issues/new), include:
...
See [Releases](https://github.com/Alos21750/ALOS-Unified-AV-Downloader/releases) for version notes and resolved issues.
