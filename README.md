...
| 單片執行緒 | 每部影片的分段工作執行緒上限為 1–16，預設依 CPU 自動取值（最多 16）；實際來源可使用較少連線，SupJav／Hanime1 直連最多 4 條。使用 Proxy 時可調低以減少負載 |
...
Hanime1 支援官方 `watch?v=` 網址、站內搜尋、分頁、排序、類型與常用標籤；下載時會即時解析站方簽名 MP4，依畫質偏好選擇可用來源，並支援最多 4 路分段、斷線續傳與單路備援。
