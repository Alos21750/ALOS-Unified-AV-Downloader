...
<p align="center">
  ...
  想自行浏览和选择视频，请使用 <strong>ALOS Browse</strong>；想按分类持续追踪新内容，请使用 <strong>ALOS Watch</strong>。<br />
  <strong>真正的核心是无人值守：</strong>设置一次分类与计划任务，之后自动发现新片、跨站去重、下载并生成字幕。
</p>

<p align="center">
  <a href="https://github.com/Alos21750/ALOS-Unified-AV-Downloader/releases/latest"><img alt="最新版本" src="https://img.shields.io/github/v/release/Alos21750/ALOS-Unified-AV-Downloader?style=flat-square&label=release&color=ff5263" /></a>
  <a href="https://github.com/Alos21750/ALOS-Unified-AV-Downloader/releases"><img alt="总下载量" src="https://img.shields.io/github/downloads/Alos21750/ALOS-Unified-AV-Downloader/total?style=flat-square&label=downloads&color=2ea44f" /></a>
  ...
  <a href="./LICENSE"><img alt="Apache 2.0 许可证" src="https://img.shields.io/github/license/Alos21750/ALOS-Unified-AV-Downloader?style=flat-square" /></a>
  <a href="https://github.com/Alos21750/ALOS-Unified-AV-Downloader/pkgs/container/alos-unified-av-downloader"><img alt="Docker amd64 和 arm64" src="https://img.shields.io/badge/Docker-amd64%20%7C%20arm64-2496ed?style=flat-square&logo=docker&logoColor=white" /></a>
</p>

<p align="center">
  <strong><a href="https://github.com/Alos21750/ALOS-Unified-AV-Downloader/releases/latest/download/ALOS_Browse.exe">下载 ALOS Browse</a></strong>
  ·
  <strong><a href="https://github.com/Alos21750/ALOS-Unified-AV-Downloader/releases/latest/download/ALOS_Watch.exe">下载 ALOS Watch</a></strong>
  ·
...
> [!TIP]
> **默认完全在本机运行，无需 API Key，也不会上传内容。** ALOS Browse 与 ALOS Watch 可在视频下载完成后自动建立播放器可切换的 `.ja.srt`、`.en.srt`、`.zh-TW.srt`，且不会修改 MP4。需要时也可自行接入常见 LLM API；云端模式只会发送识别后的字幕文字以及必要的 API 验证和常规连接信息，绝不会上传视频或音频。

<p align="center">
  <img src="./docs/assets/alos-browse.png" width="100%" alt="ALOS Unified AV Downloader ALOS Browse 深色界面，包含 JableTV、MissAV 与 SupJav 浏览标签" />
</p>
...
| 需求 | 建议 | 操作方式 |
|---|---|---|
| 浏览、搜索并逐个选择视频 | **ALOS_Browse.exe** | 浏览视频卡片、多选、加入队列或立即下载 |
| 让电脑无人值守自动追新 | **ALOS_Watch.exe** | 选择网站、分类、日期、版本优先级与计划任务；之后自动扫描、去重、下载并生成字幕 |
| Defender 对单文件 ALOS Watch 报告检测 | **ALOS_Watch_portable.zip** | 先核对哈希和检测详情，再评估无需临时自解压的备用包；若备用包也被检测，请停止使用并报告 |
| 在 NAS／服务器上无界面运行 | **Docker / CLI** | 传入一个或多个网址，或挂载 `urls.txt` |

如果不确定，请先使用 **ALOS Browse**。两个 Windows 可执行文件均无需安装 Python，Release 版本已经包含 ffmpeg。

深入了解：[ALOS Browse](./docs/alos-browse.md) · [ALOS Watch 无人值守流程](./docs/alos-watch.md) · [AI 字幕](./docs/ai-subtitles.md) · [Docker / CLI](./docs/docker-cli.md) · [v3 迁移](./docs/migration-v3.md)

## Windows：30 秒开始

1. 下载 [ALOS_Browse.exe](https://github.com/Alos21750/ALOS-Unified-AV-Downloader/releases/latest/download/ALOS_Browse.exe) 或 [ALOS_Watch.exe](https://github.com/Alos21750/ALOS-Unified-AV-Downloader/releases/latest/download/ALOS_Watch.exe)。
...
## ALOS Browse：浏览、选择、下载

1. 在“浏览”中选择 JableTV、MissAV、SupJav 或 Hanime1，然后选择分类或输入关键词。
...
## ALOS Watch：按分类自动追踪新内容

<p align="center">
  <img src="./docs/assets/alos-watch.png" width="100%" alt="Jable ALOS Watch 深色界面，显示 MissAV 分类、日期、画质、版本优先级与 AI 字幕" />
</p>

1. 选择保存位置；若留空，ALOS Watch 会在可执行文件旁自动建立 `tmp`。
2. 点击“显示设置”调整基准日期、画质、单片线程、版本优先级、AI 字幕和 Proxy；平时收起设置可将窗口空间留给分类。
...
| Hanime1 | 24 | 上市／上传／排行、9 种视频类型和中文字幕等特色标签 |

ALOS Watch 可设置为每 1–168 小时自动检查，或每天按这台电脑的本地时间在指定时刻检查；旧设置仍默认为每 24 小时。点击“立即检查”会马上运行一次，不会建立重复计划。同一番号在不同分类或网站重复出现时，会优先保留符合用户版本偏好的候选项；无法可靠识别番号时仅按完全相同的网址去重，不会猜测合并。

下载记录优先保存在可执行文件旁的 `.alos-watch`；如果该位置不可写，则改用 `%APPDATA%\ALOS Unified AV Downloader\watch`。
...
- ALOS Browse 的视频下载与字幕处理使用独立队列；字幕逐个在后台生成，不占用 1–32 个视频下载名额。
- 生成速度取决于 CPU、视频长度、实际语音比例和所选翻译服务；三语共用同一次本机语音识别。本机模式也会复用中间翻译结果，避免重复推理。

## 支持范围

| 网站 | ALOS Browse 浏览／搜索／下载 | ALOS Watch 分类监控 | Docker／CLI 网址下载 |
|---|:---:|:---:|:---:|
| JableTV | ✓ | ✓ | ✓ |
| MissAV | ✓ | ✓ | ✓ |
| SupJav | ✓ | ✓ | ✓ |
...
Hanime1 支持官方 `watch?v=` 网址、站内搜索、分页、排序、类型和常用标签；下载时会实时解析站方签名 MP4，按画质偏好选择可用来源，并支持最多 4 路分段、断线续传和单路备用。
...
公开镜像为 `ghcr.io/alos21750/alos-unified-av-downloader:latest`，GitHub Actions 会构建 amd64 和 arm64 版本。
...
Docker 是无界面、执行完即退出的下载任务，不包含 ALOS Browse 或 ALOS Watch GUI。

## 遇到问题

提交 [GitHub Issue](https://github.com/Alos21750/ALOS-Unified-AV-Downloader/issues/new) 时，请提供：
...
需要 Proxy 时，可在 ALOS Browse“设置”或 ALOS Watch 顶部选择自定义代理、Windows 系统代理或禁用。两个 GUI 共用该设置，而且只作用于本程序；Windows 模式目前支持已启用的手动 ProxyServer。PAC 配置网址会被提示但不会执行，尚不支持 WPAD 自动检测。

## Stars 与项目活动

<p align="center">
  <img src="./docs/assets/star-history.svg" width="100%" alt="ALOS Unified AV Downloader 已验证的 GitHub Star 历史" />
</p>
...
版本变更与已解决的问题请查看 [Releases](https://github.com/Alos21750/ALOS-Unified-AV-Downloader/releases)。
