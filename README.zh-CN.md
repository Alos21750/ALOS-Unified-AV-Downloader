...
<p align="center">
  面向 JableTV、MissAV、SupJav 的桌面下载器与自动监控工具，内置 <strong>AI 生成字幕</strong>。<br />
  <strong>视频下载完成后自动添加 AI 字幕：</strong>日语音轨在本机识别，可输出日文、英文与繁体中文 SRT。<br />
  想自行浏览和选择视频，请使用 <strong>Modern</strong>；想按分类持续追踪新内容，请使用 <strong>SmallTool</strong>。
</p>

<p align="center">
  <a href="https://github.com/Alos21750/JableTV-MissAV-Downloader-GUI-2026/releases/latest"><img alt="最新版本" src="https://img.shields.io/github/v/release/Alos21750/JableTV-MissAV-Downloader-GUI-2026?style=flat-square&label=release&color=ff5263" /></a>
  <a href="https://github.com/Alos21750/JableTV-MissAV-Downloader-GUI-2026/releases"><img alt="总下载量" src="https://img.shields.io/github/downloads/Alos21750/JableTV-MissAV-Downloader-GUI-2026/total?style=flat-square&label=downloads&color=2ea44f" /></a>
  ...
  <a href="./LICENSE"><img alt="Apache 2.0 许可证" src="https://img.shields.io/github/license/Alos21750/JableTV-MissAV-Downloader-GUI-2026?style=flat-square" /></a>
  <a href="https://github.com/Alos21750/JableTV-MissAV-Downloader-GUI-2026/pkgs/container/jabletv"><img alt="Docker amd64 和 arm64" src="https://img.shields.io/badge/Docker-amd64%20%7C%20arm64-2496ed?style=flat-square&logo=docker&logoColor=white" /></a>
</p>

<p align="center">
  <strong><a href="https://github.com/Alos21750/JableTV-MissAV-Downloader-GUI-2026/releases/latest/download/JableTV_Modern.exe">下载 Modern</a></strong>
  ·
  <strong><a href="https://github.com/Alos21750/JableTV-MissAV-Downloader-GUI-2026/releases/latest/download/Jable_smalltool.exe">下载 SmallTool</a></strong>
  ·
...
> [!TIP]
> **默认完全在本机运行，无需 API Key，也不会上传内容。** Modern 与 SmallTool 可在视频下载完成后自动建立播放器可切换的 `.ja.srt`、`.en.srt`、`.zh-TW.srt`，且不会修改 MP4。需要时也可自行接入常见 LLM API；云端模式只会发送识别后的字幕文字以及必要的 API 验证和常规连接信息，绝不会上传视频或音频。

<p align="center">
  <img src="./img/readme_modern.png" width="100%" alt="JableTV Downloader Modern 深色界面，包含 JableTV、MissAV 与 SupJav 浏览标签" />
</p>
...
| 需求 | 建议 | 操作方式 |
|---|---|---|
| 浏览、搜索并逐个选择视频 | **JableTV_Modern.exe** | 浏览视频卡片、多选、加入队列或立即下载 |
| 持续追踪指定分类的新内容 | **Jable_smalltool.exe** | 选择网站、分类、日期和版本优先级后开始监控 |
| Defender 对单文件 SmallTool 报告检测 | **Jable_smalltool_portable.zip** | 先核对哈希和检测详情，再评估无需临时自解压的备用包；若备用包也被检测，请停止使用并报告 |
| 在 NAS／服务器上无界面运行 | **Docker / CLI** | 传入一个或多个网址，或挂载 `urls.txt` |

如果不确定，请先使用 **Modern**。两个 Windows 可执行文件均无需安装 Python，Release 版本已经包含 ffmpeg。

## Windows：30 秒开始

1. 下载 [JableTV_Modern.exe](https://github.com/Alos21750/JableTV-MissAV-Downloader-GUI-2026/releases/latest/download/JableTV_Modern.exe) 或 [Jable_smalltool.exe](https://github.com/Alos21750/JableTV-MissAV-Downloader-GUI-2026/releases/latest/download/Jable_smalltool.exe)。
...
1. 在“浏览”中选择 JableTV、MissAV 或 SupJav，然后选择分类或输入关键词。
...
| 单片线程 | 每个视频的分段工作线程上限为 1–16，默认根据 CPU 自动计算（最高 16）；实际来源可能使用更少连接，SupJav 直连最多 4 条。使用 Proxy 时可调低以减轻负载 |
...
## SmallTool：按分类自动追踪新内容

<p align="center">
  <img src="./img/readme_smalltool.png" width="100%" alt="Jable SmallTool 深色界面，显示 MissAV 分类、日期、画质、版本优先级与 AI 字幕" />
</p>

1. 选择保存位置；若留空，SmallTool 会在可执行文件旁自动建立 `tmp`。
2. 点击“显示设置”调整基准日期、画质、单片线程、版本优先级、AI 字幕和 Proxy；平时收起设置可将窗口空间留给分类。
3. 在三个网站标签中搜索并勾选分类；支持整组全选。
...
SmallTool 可设置为每 1–168 小时自动检查，或每天按这台电脑的本地时间在指定时刻检查；旧设置仍默认为每 24 小时。点击“立即检查”会马上运行一次，不会建立重复计划。同一番号在不同分类或网站重复出现时，会优先保留符合用户版本偏好的候选项；无法可靠识别番号时仅按完全相同的网址去重，不会猜测合并。

下载记录优先保存在可执行文件旁的 `.Jable_smalltool`；如果该位置不可写，则改用 `%APPDATA%\JableTV Downloader\smalltool`。
...
- Modern 的视频下载与字幕处理使用独立队列；字幕逐个在后台生成，不占用 1–32 个视频下载名额。
- 生成速度取决于 CPU、视频长度、实际语音比例和所选翻译服务；三语共用同一次本机语音识别。本机模式也会复用中间翻译结果，避免重复推理。

## 支持范围

| 网站 | Modern 浏览／搜索／下载 | SmallTool 分类监控 | Docker／CLI 网址下载 |
|---|:---:|:---:|:---:|
| JableTV | ✓ | ✓ | ✓ |
| MissAV | ✓ | ✓ | ✓ |
| SupJav | ✓ | ✓ | ✓ |
...
公开镜像为 `ghcr.io/alos21750/jabletv:latest`，GitHub Actions 会构建 amd64 和 arm64 版本。
...
Docker 是无界面、执行完即退出的下载任务，不包含 Modern 或 SmallTool GUI。

## 遇到问题

提交 [GitHub Issue](https://github.com/Alos21750/JableTV-MissAV-Downloader-GUI-2026/issues/new) 时，请提供：
...
需要 Proxy 时，可在 Modern“设置”或 SmallTool 顶部选择自定义代理、Windows 系统代理或禁用。两个 GUI 共用该设置，而且只作用于本程序；Windows 模式目前支持已启用的手动 ProxyServer。PAC 配置网址会被提示但不会执行，尚不支持 WPAD 自动检测。

## Stars 与项目活动

<p align="center">
  <img src="./img/star-history.svg" width="100%" alt="JableTV Downloader 已验证的 GitHub Star 历史" />
</p>
...
版本变更与已解决的问题请查看 [Releases](https://github.com/Alos21750/JableTV-MissAV-Downloader-GUI-2026/releases)。
